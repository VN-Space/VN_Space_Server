...
import {
    ...
    BaseError,
    ErrorHandler,
    HttpStatusCode, InternalServerErrorException,
} from '../common';
...
export class App extends BaseController {
    ...
    private connectMongo(mongoURL: string) {
        mongoose
            .connect(mongoURL)
            .then(() => logger.info('[MongoDB] Connected to database system.'))
            .catch(err => {
                throw new InternalServerErrorException(err ? err : '[MongoDB] Cannot connect to the database system.');
            });
    }
